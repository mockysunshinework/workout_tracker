class WorkoutSet < ApplicationRecord
  ...
  # 上限 1000 は実用上の決め値（SPEC 4.5。列型 decimal(5,1) の格納上限は 9999.9）。
  # DB の CHECK 制約と二層で担保する。NULL の可否は種目の bodyweight で決まるため別検証。
  ...
  # exercise が未設定でも本検証は実行される（belongs_to の presence エラーとは独立）ため
  # セーフナビゲーションで nil に備える。
