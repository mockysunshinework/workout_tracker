...
#### スキップは success で終わる（2026-08-07・評価軸5に該当）

PR #21（141 行・3 ファイル）で、`auto-review` が **5 ターン・22 秒・$0.20 で投稿0件のまま success** で終了した。プラグイン手順1のスキップ判定に当たったと考えられる。

- closed / draft / コメント済みのいずれにも該当しない（OPEN・非 Draft・コメント0件）ため、残る条件は「些末で明らかに正しい変更」
- 同じ PR をローカルで実行すると**スキップされず完走した**（指摘なし）。この判定は haiku エージェントによる**非決定的な判断**である
- **ジョブは success で終わるため、投稿の有無を見ないと「指摘なし」と区別がつかない**

[12.4](#124-評価軸) の評価軸5（不適切な skip、目安ゼロ）に **1 件目を計上**する。

回避策は PR の close → reopen（`reopened` で再実行）またはローカル実行。運用注意は `docs/branching-rules.md` 7.2 に記載した。

#### `mention` ジョブの `--max-turns`（2026-08-07）

PR #21 で `@claude レビューして下さい` と依頼したところ、`error_max_turns`（21 ターン / 上限 20）で失敗した。`permission_denials_count` は **12**。

`mention` はプラグインを経由しない素の Claude Code のため、レビューを依頼すると許可外のツール（`bundle exec rspec` 等）を試みて拒否され、そのたびターンを消費する。上限を 40 に引き上げたが、**`mention` は質問用途に使い、レビューは `auto-review` に任せる**のが本来の役割分担である。
...
- 2.5（2026-08-07）: スキップが success で終わる事象（評価軸5の1件目）と、`mention` ジョブの `--max-turns` 不足を 12.5b に記録。`mention` の上限を 20 → 40 に引き上げ、役割を質問用途に限定する方針を明記。
