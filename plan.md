...
- [x] 7.6 エラー分類応答の実装
  ...
  - 実施結果（2026-09-01）: parse 段階のみを begin/rescue で囲む構造に変更。7.4/7.5 の triage で積んだ既知の穴（正署名＋不正本文 → 500）を解消
    - **200（業務エラー相当）**: 正署名だが本文がパース不能なケース。`JSON::ParserError`（壊れた JSON）と `NoMethodError`（JSON だが Webhook 形式でない）を個別 example で検証（triage の方針どおり）。ログには例外クラスのみ記録（本文は個人情報を含みうるため出さない・CLAUDE.md）
    - **500（一時障害）**: 業務処理中の例外は**あえて rescue しない**（LINE 再送 → 冪等性でリカバリ）。`rescue StandardError` の範囲を parse 段階に限定した理由（イベント処理まで覆うと業務例外を 200 で握りつぶし再送リカバリが壊れる）をコメントに明記。DB 例外（ConnectionNotEstablished）→ 500 の spec は**実装前から通る回帰ガード**（未 rescue = 現状挙動のため RED 対象外。将来 rescue を広げすぎた場合に検出する）
    - **400（署名不正）**: 7.4 の既存 spec が担保
    - spec 側は `show_exceptions = :all` に around で切り替えて「本番で 500 応答になる」ことをステータスとして検証（test 既定の :rescuable では例外が raise されるため）
    - 「案内返信」は返信基盤（9 章）の実装後に業務エラーへ付加する（現段階では返信先が存在しないケースのみのため対象外）
    - Brakeman 実行（Webhook 変更のため）: 警告 0
  - レビュー確認（2026-09-01・`@claude` メンションレビュー）: ブロッカーなし・Approve 表明。指摘 2 件とも現状維持
    - 「`rescue StandardError` が広い（明示列挙も一案）」: 論点は正当だが**明示列挙には実証済みの穴がある** — 7.5b 開発中、`follow` プロパティ欠如の本文で SDK が `ArgumentError`（missing keyword）を投げることを実際に確認しており、不正本文の例外クラスは列挙で網羅できない。「範囲を parse 段階に絞った上でクラスは広く受ける」が両リスク（握りつぶし vs 無意味な再送）のバランス点
    - 「`request.body.read` も rescue 範囲内（I/O 例外が 200 になる）」: 事実だが見送り — 発生が極めて稀・request spec で再現不能（テストなしの挙動変更を避ける）・ログに例外クラスは残る
- [ ] 7.7 Webhook 再送の有効化
...
- [x] 8.2b LINE 連携設定画面の実装（UI）
  ...
  - 実施結果（2026-09-03）: `Settings::LineController`（show / link_code / destroy）。ルートは `GET・DELETE /settings/line`＋`POST /settings/line/link_code`（singular resource＋member post）。`current_user` への操作のみで他ユーザー資源の取得なし
    - User に表示用の問い合わせメソッドを追加: `line_linked?` / `line_link_code_active?`（未消費かつ期限内）
    - 画面: 連携状態（済み/未）で表示を分岐。未連携時は手順・有効期限内のコード・発行/再発行ボタン。連携済み時は解除ボタン（`turbo_confirm` 付き）。ダッシュボードのナビに導線を追加
