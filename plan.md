...
- [ ] 5.5 バリデーションエラー等の日本語化（i18n）
  - 実施内容: `default_locale` を `:ja` にし、Rails 標準・Devise・モデル名/属性名の日本語 locale を整備する（`rails-i18n` / `devise-i18n` 等の gem 導入の要否はここで判断し提案する）。flash に表示される `errors.full_messages` が日本語になることを確認する
