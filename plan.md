...
- [x] 4.4 セット採番と競合対策の実装
  ...
  - レビュー指摘に対応（2026-08-18・`@claude` メンションレビュー）: `**attributes` に `set_number` を含めると後勝ちで自動採番が黙って上書きされる穴を修正（自動採番側を後置）。「渡しても無視される」spec を追加し不変条件を固定。もう1件の指摘（並行 spec の `after` が全件削除）は「非トランザクショナルなテストが増えた場合はテーブルの奪い合いに注意」という将来向けの注記で、現状1ファイルのみ・並列実行なしのため対応不要と判断
- [x] 4.5 セット削除時の繰り上げ処理の実装
...
- [x] 5.4 種目管理画面
  ...
  - 作業分解（tdd-dev 実行管理用）:
    - [x] 一覧表示（自分の独自種目＋プリセット。他ユーザーの種目は非表示）
    - [x] 独自種目の追加（正規化重複はエラー）
    - [x] 名称変更（自分の種目のみ。プリセット・他ユーザーは 404）
    - [x] 削除（使用中は拒否・4.3 の削除制限）
  - 実施結果（2026-08-21）: `resources :exercises, only: [:index, :create, :update, :destroy]` と `ExercisesController` を追加。User に `has_many :exercises, dependent: :destroy` を追加（**宣言順に依存あり**: workouts が先に消えることで exercises の削除制限と衝突しない。コメントに明記）
    - 変更・削除は `current_user.exercises.find` 起点のため、プリセット（user_id NULL）と他ユーザーの種目は自然に 404
    - 名称変更は `params.expect(exercise: [:name])` で name のみ（仕様 4.4 画面 5 が「名称変更」のみ規定。category / bodyweight の変更は不可 — bodyweight 変更は既存記録の weight 検証と整合しなくなるため）
    - カテゴリは自由入力・任意（仕様はユーザー種目のカテゴリを 6 分類に限定していないため。プリセット準拠の select 化は必要になった時点で検討）
    - spec 12 examples: 認証ガード / 一覧（プリセット＋自分・他ユーザー非表示）/ 追加（正規化重複 `ベンチぷれす` はエラー）/ 名称変更（プリセット・他ユーザー 404）/ 削除（**使用中は拒否**・プリセット・他ユーザー 404）
  - 5 章完了時のセキュリティチェック（2026-08-21）: bundler-audit（脆弱性なし）/ bundle check（整合）/ Brakeman（警告なし）/ 差分への秘密情報混入なし。全体テスト 166 examples green・RuboCop no offenses
  - レビュー指摘に対応（2026-08-23・`@claude` メンションレビュー）: (1) **削除ボタンに `turbo_confirm` を追加**（種目・セットとも。物理削除の誤操作防止。RED で属性なしを確認してから実装）。spec は「属性が削除ボタンに付与される」ことを検証（ダイアログ表示自体は Turbo のクライアント挙動で request spec では検証不可。テスト名もその範囲に合わせた）。(2) エラーメッセージの英語混在（i18n 未対応）は既存パターン踏襲で本 PR の劣化ではないため本 PR では対応せず、**5.5 として計画に追加**（ユーザー指示）
- [x] 5.5 バリデーションエラー等の日本語化（i18n）
  - 実施内容: `default_locale` を `:ja` にし、Rails 標準・Devise・モデル名/属性名の日本語 locale を整備する（`rails-i18n` / `devise-i18n` 等の gem 導入の要否はここで判断し提案する）
  - 完了条件: 画面に表示されるバリデーションエラー・Devise メッセージが日本語で表示され、既存 spec が green のまま
  - 経緯: PR #42 レビューの所見（エラーメッセージの英語混在）を受けて追加（2026-08-23・ユーザー指示）
  - 決定（2026-08-23・ユーザー承認）: **`rails-i18n`（8.1.0）＋ `devise-i18n`（1.16.0）を導入**。Rails 標準エラー全種・Devise 全文言の手書きと将来の追従保守を避け、アプリ固有のモデル名・属性名のみ自前の `ja.yml` に書く
...
- [x] 6.1 グラフ用集計の実装
  ...
  - 作業分解（tdd-dev 実行管理用）:
    - [x] 種目別推移の集計（ExerciseProgress: 日付ごとの最大重量・推定 1RM）
    - [x] 月間頻度の集計（TrainingFrequency: 週/月ごとの実施日数）
  - 実施結果（2026-08-24）: `app/models/exercise_progress.rb`（5 examples）・`app/models/training_frequency.rb`（5 examples）を追加（3.3 の正規化と同じ「app/models 配下の単独モジュール」パターン。集計は SQL 側の GROUP BY で行う）
    - **推定 1RM は「セットごとの e1RM の日次最大」**であり最大重量セットの e1RM ではない。取り違えを検出するデータ（100×10 の e1RM 133.3 ＞ 最大重量 102.5×3 の 112.8）で検証。値は小数 1 桁に丸め
    - **重量未入力（自重）のセットは種目別推移の対象外**（縦軸が重量のため。仕様に明記がなく実装詳細として判断）
    - 月間頻度は `date_trunc`（week/month）で集計。**週は月曜起点（ISO 週）**。unit はホワイトリスト検証（SQL 文字列に埋め込むため。Brakeman 警告なしを確認）
